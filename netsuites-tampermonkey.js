...
(function(){
    ...
    var styles = [
        ...
        "#customFileBrowser { display: none; position: absolute; top: 10%; left: 0; right: 0; bottom: 0; background: white; width: 75%; height: 75%; margin: 0 auto; padding: 16px; overflow: scroll; z-index: 1000; }",
    ...
    */
    ...
    var ensureTableIsPopulated = function(){
        ...
        var $table = jQuery(parent.document.getElementById("div__body"));
        var $editLinks = $table.find("tr>td:nth-child(1):visible a");
        var $existingLinks = $table.find(".direct-edit");

        //Retrieving the index of the 'Type' header
        var typeIndex = 1 + jQuery('#div__labtab td:contains(Type)').index();
        var typeSelector = "td:nth-child(" + typeIndex + ")";

        for(var i = 0, l = $editLinks.length; i < l; i++){
            var $cell = $editLinks.eq(i);
            var type = jQuery.trim($cell.parents('tr').eq(0).find(typeSelector).text());
            if(type !== "Folder" && type !== ""){
                var href = $cell.attr('href');
                var cellId = getParam('id', href.substring(href.indexOf('?')));
    ...
    */
    var initializeFileBrowser = function(){
        ...
        $fileBrowser.find('#browserContent').split({
    ...
    */
    ...
    jQuery(document).ready(function(){
        ...
        if( isValidPage() ){
            ...
            generateButton("FileBrowser", toggleFileBrowser);
